...
const StaffDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const childrenData = await mockApi.getChildren(user.id);
      ...
      // Fetch schedule
      const scheduleData = await mockApi.getScheduleByStaff(user.id);
      ...
      <Paper elevation={3} sx={{ p: 3, mb: 4, display: 'flex', alignItems: 'center' }}>
        ...
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            {user?.bio || 'Staff Member at Daystar Daycare'}
          </Typography>
          <Button
            variant="outlined"
            startIcon={<EditIcon />}
